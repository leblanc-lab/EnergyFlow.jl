    ...
    bool norm = false)
{
    ...
    EMDFloat64<> emd_calculator;
    emd_calculator.set_R(R);
    emd_calculator.set_beta(beta);
    emd_calculator.set_norm(norm);

    // Create events - VectorEvent expects flat particle coordinates
    // coords1 and coords2 are already flat: [y1, phi1, y2, phi2, ...]
    DefaultEvent ev1(coords1, weights1);
    DefaultEvent ev2(coords2, weights2);
